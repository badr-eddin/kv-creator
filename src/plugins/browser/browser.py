...
class FileDialog(QWidget):
    ...
    def __init__(self, main=None, std=None, **kwargs):
        ...
        self.widget = std.import_("plugins/fbr/file-dialog.ui")
        self.creating = False
        self._close = False
    ...
    def config_ui(self):
        ...
        self.widget.bclose.setIcon(QIcon(self.std.import_("img/widgets/close.png")))
    ...
    def _open_save_clicked(self):
        ...
        if self._data.get("mode") == self.Mode.Save:
            ...
            if not k:
                ...
                self.cover.close()
                self.callback(paths)  # bad-feature -> if len(paths) > 1 else paths[0])
    ...
    def is_usable(std):
        try:
            std.import_("plugins/fbr/file-dialog.ui")
            return True
        except Exception as e:
            _ = e
            return False
    ...
    def navigate_to(self, path=None):
        ...
        if path.is_dir():
            ...
            self.cover.close()
            path = self.encapsulate(path)
            self.callback(path if self._data.get("selection") == self.Selection.Single else [path])
